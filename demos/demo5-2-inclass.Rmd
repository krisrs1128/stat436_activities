...
th <- theme_minimal() + 
  theme(
    ...
    axis.title = element_text(linewidth = 16),
...
scatterplot(rentals, c(TRUE, rep(FALSE, nrow(rentals) - 1)))
overlay_histogram(rentals, rep(TRUE, nrow(rentals)))
```
...
ui <- fluidPage(
  ...
  fluidRow(
    column(6,
      plotOutput("histogram", brush = brushOpts("plot_brush", height = 200)), # render got called -> update histogram
      dataTableOutput("table") # render called -> update table
    ),
    column(6, plotOutput("map", brush = "plot_brush", height = 600)), # render called, update map
  ),
...
server <- function(input, output) {
  ...
  observeEvent(
      input$plot_brush,
      # user draws brush -> update the selection
      selected(
          brushedPoints(rentals, input$plot_brush, allRows = TRUE)$selected_
      )
  )
  
  output$map <- renderPlot(scatterplot(rentals, selected())) # selected changes, rerun Plot
  output$table <- renderDataTable(rentals |> filter(selected())) # selected changes, rerun Table
  output$histogram <- renderPlot(overlay_histogram(rentals, selected())) # same with histogram
}
