...
reviews
#z <- embed_text(reviews$Review, model = model_name)
z <- read_csv("~/Downloads/z.csv")
```
...
```{r}
umap_recipe <- recipe(~ ., data = z) |>
  step_umap(all_predictors())

scores <- prep(umap_recipe) |>
  juice() |>
  bind_cols(reviews)

ggplot(scores) +
  geom_point(aes(UMAP1, UMAP2, col = factor(Rating)))
...
```{r}
ui <- fluidPage(
    titlePanel("Interactive UMAP Visualization"),
    sidebarLayout(
        sidebarPanel(
            helpText("Brush points on the scatterplot to see review details.")
        ),
        mainPanel(
            plotOutput("umapPlot", brush = "plot_brush"),
...
server <- function(input, output) {
    selected <- reactiveVal(rep(FALSE, nrow(scores)))    
    ...
    output$reviewTable <- renderTable({
        browser()
        brushed <- brushedPoints(scores, input$plot_brush, allRows = TRUE)$selected_
        selected(brushed)
        scores$Review[selected()]
    })
}
