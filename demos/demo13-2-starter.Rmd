...
library(glue)
imgs <- glue("~/Downloads/subset/{py$fnames[1:10]}") |>
  map(brick)

map(imgs, ~ tm_shape(.) + tm_rgb())
```
...
library(ggimage)
scores_files <- scores |>
  mutate(image = glue("~/Downloads/subset/{py$fnames}"))

ggplot(scores_files, aes(x = UMAP1, y = UMAP2)) +
