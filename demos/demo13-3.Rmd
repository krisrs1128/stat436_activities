...
#z <- embed_text(reviews$Review, model = model_name, verbose = TRUE)
z <- read_csv("https://uwmadison.box.com/shared/static/8j0um7k06qbu0nozks1i2mwzmahn7cpw.csv")
```
...
```{r}
rec <- recipe(~ ., data = z) |>
    step_umap(all_predictors(), neighbors = 25)

umap_result <- prep(rec)
scores <- juice(umap_result) |>
    bind_cols(reviews)

p <- ggplot(scores, aes(UMAP1, UMAP2, text = Review, col = factor(Rating))) +
    geom_point()
```
...
server <- function(input, output, session) {
    selected <- reactiveVal(rep(FALSE, nrow(reviews)))

    output$scatterplot <- renderPlot(p)
    observeEvent(input$plot_brush, {
        ix <- brushedPoints(scores, input$plot_brush, allRows = TRUE)$selected_
        selected(ix)
    })

    output$reviews <- renderTable({
        reviews$Review[which(selected())]
    })
}
