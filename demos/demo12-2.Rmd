...
ggplot(posterior) +
  ...
  geom_point(data = x, aes(propsurv, tank), shape = 1) +
  facet_wrap(size ~ density, scales = "free_y") +
...
y_sim |>
  ggplot() +
    geom_jitter(aes(y_sim, reorder(tank, y_sim), col = pred), position = position_jitter(h = 0.2, w = .3), size = 0.2) +
...
anim_save("animation2.gif", p, nframes = 20, height = 5, width = 10, units = "in", res = 450, renderer = magick_renderer())
